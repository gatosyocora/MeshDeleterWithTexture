...
float3 hsvLerp(float3 startHSV, float3 endHSV, float t)
{
    float3 hsv;
    t = clamp(t, 0, 1);
    startHSV.x = frac(startHSV.x);
    endHSV.x = frac(endHSV.x);
    float sub = endHSV.x - startHSV.x;
    sub -= floor(sub);
    if (sub > 0.5)
        sub -= 1.0;
    if (sub < -0.5)
        sub += 1.0;
    float delta = sub * t;
    hsv.x = frac(1.0 + startHSV.x + delta);
    hsv.yz = lerp(startHSV.yz, endHSV.yz, t);
    return hsv;
}
/*
** HSL
...
float HueToRGB(float f1, float f2, float hue)
{
    ...
    res = f1;
    res = (c1) ? f1 + (f2 - f1) * 6.0 * hue : res;
    res = (!c1 && c2) ? f2 : res;
    res = (!c1 && !c2 && c3) ? f1 + (f2 - f1) * (0.666666666 - hue) * 6.0 : res;
    return res;
}
//float HueToRGB(float f1, float f2, float hue)
//{
...
{
    float3 rgb1, rgb2;
    rgb1 = float3(hsl.z, hsl.z, hsl.z); // Luminance
    float f2 = hsl.z < 0.5 ? hsl.z * (1.0 + hsl.y) : (hsl.z + hsl.y) - (hsl.y * hsl.z);
    float f1 = 2.0 * hsl.z - f2;
    rgb2.r = HueToRGB(f1, f2, hsl.x + 0.3333333333);
    rgb2.g = HueToRGB(f1, f2, hsl.x);
    rgb2.b = HueToRGB(f1, f2, hsl.x - 0.3333333333);
    return hsl.y == 0? rgb1 : rgb2;
}
//float3 HSLToRGB(float3 hsl)
//{
//    float3 rgb;
...
{
    return (blendColor < 0.5) ? 2.0 * baseColor * blendColor : 1.0 - 2.0 * (1.0 - blendColor) * (1.0 - baseColor);
}
float3 BlendHardLight(float3 baseColor, float4 blendColor)
{
    return lerp(baseColor, BlendHardLight(baseColor, blendColor.rgb), blendColor.a);
}
